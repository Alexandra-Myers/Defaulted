...
public record EnchantmentModifierGenerator(Object2IntOpenHashMap<Holder<Enchantment>> entries) implements PatchGenerator {
    ...
    public static final MapCodec<EnchantmentModifierGenerator> CODEC = Codec.unboundedMap(BuiltInRegistries.ENCHANTMENT.holderByNameCodec(), LEVEL_CODEC)
    ...
    public void patchDataComponentMap(Item item, PatchedDataComponentMap patchedDataComponentMap) {
        ...
        entries.forEach((enchantmentHolder, level) -> enchantments.set(enchantmentHolder.value(), level));
