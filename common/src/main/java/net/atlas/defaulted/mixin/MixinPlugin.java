...
import net.atlas.defaulted.annotation.Incompatible;
import net.atlas.defaulted.annotation.ModSpecific;
...
public class MixinPlugin implements IMixinConfigPlugin {
	...
	public boolean shouldApplyMixin(String targetClassName, String mixinClassName) {
		Logger logger = LogManager.getLogger("Defaulted");
		try {
		...
		for (int i = 0; i < values.size(); i += 2) {
			...
			if ("value".equals(pKey) && pValue instanceof List) {
				for (String modId : (List<String>) pValue) {
					if (DefaultedExpectPlatform.isModLoaded(modId)) {
						return modId;
					}
				}
			}
